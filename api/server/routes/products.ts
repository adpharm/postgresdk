...
import { z } from "zod";
import { InsertProductsSchema, UpdateProductsSchema } from "../zod/products";
import { loadIncludes } from "../include-loader";
import * as coreOps from "../core/operations";


const columnEnum = z.enum(["id", "name", "status", "priority", "tags"]);
...
/**
 * Register all CRUD routes for the products table
 ...
 */
export function registerProductsRoutes(app: Hono, deps: { pg: { query: (text: string, params?: any[]) => Promise<{ rows: any[] }> }, onRequest?: (c: Context, pg: { query: (text: string, params?: any[]) => Promise<{ rows: any[] }> }) => Promise<void> }) {
  ...
  app.post(base, async (c) => {
    ...
    const parsed = InsertProductsSchema.safeParse(body);
  ...
  app.patch(`${base}/:id`, async (c) => {
    const pkValues = [c.req.param("id")];
    const body = await c.req.json().catch(() => ({}));
    const parsed = UpdateProductsSchema.safeParse(body);
