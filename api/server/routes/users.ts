...
import { z } from "zod";
import { InsertUsersSchema, UpdateUsersSchema } from "../zod/users";
import { loadIncludes } from "../include-loader";
import * as coreOps from "../core/operations";


const columnEnum = z.enum(["id", "email", "role", "backup_role"]);
...
/**
 * Register all CRUD routes for the users table
 ...
 */
export function registerUsersRoutes(app: Hono, deps: { pg: { query: (text: string, params?: any[]) => Promise<{ rows: any[] }> }, onRequest?: (c: Context, pg: { query: (text: string, params?: any[]) => Promise<{ rows: any[] }> }) => Promise<void> }) {
  const base = "/v1/users";
  ...
  app.post(base, async (c) => {
    ...
    const parsed = InsertUsersSchema.safeParse(body);
  ...
  app.patch(`${base}/:id`, async (c) => {
    const pkValues = [c.req.param("id")];
    const body = await c.req.json().catch(() => ({}));
    const parsed = UpdateUsersSchema.safeParse(body);
