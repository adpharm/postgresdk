...
- fix: Nullable belongs-to relationships now correctly typed as `T | null` in generated SDK
  - When a FK column is nullable, the `Edge` in the relationship graph is marked `nullable: true`
  - Include method types, include resolver types, and combined include types all emit `| null` for nullable one-to-one edges
  - Previously, nullable FK columns produced non-null types, causing type mismatches at runtime
...
- feat: Soft/hard delete config consolidated under `delete` block; `hardDelete()` method added
  ...
  - Setting `exposeHardDelete: false` prevents permanent deletion through the API — only soft deletes are possible
  - `deleteRecord` in generated server code now accepts `opts.hard` to bypass soft-delete logic when `exposeHardDelete` is enabled
  - Generated tests updated to use `hardDelete()` instead of `delete()` when cleaning up foreign-key test data
...
- feat: `pull` command now deletes stale files from the output directory
  - Files present on disk but no longer in the SDK bundle are automatically removed after each pull
  - Works recursively across all subdirectories under the output path
  - Deleted files are logged with `✗ <path>`; summary line reports count of deleted stale files
...
- feat: Add stale file cleanup to `generate()` — removes files for tables no longer in the schema
  - New `clean` config option (default: `true`) to opt out of automatic cleanup
  - Scans server, client, and test output dirs and deletes `.ts`, `.md`, `.yml`, `.sh` files not in the current generation set
  - Summary log line now includes deleted file count alongside updated/unchanged counts
- fix: Emit `distinctOn` param in generated client TypeScript types
  - `distinctOn?: string | string[]` is now included in list query param types across all client emit patterns
...
- feat: Extend TypeScript overloads to JSONB tables
  - Tables with JSONB columns now have type-safe overloads for select/exclude
  - Generic type parameter preserved across all overload signatures
  - Consistent type safety for both JSONB and non-JSONB tables
  - Improves IDE experience when working with JSONB fields
...
- feat: Add version-based cache invalidation
  - Cache now includes postgresdk version in schema hash
  - Automatically regenerates code when upgrading postgresdk
  - Ensures generated code matches installed version
  - Prevents stale code after package upgrades
